...
use App\Engine\Performance\Deperdition\DeperditionBaie;
use App\Engine\Performance\Functions;
use App\Engine\Performance\Rule;

final class EnsoleillementLnc extends Rule
{
    private Lnc $lnc;
    ...
    public function fe(Mois $mois): float
    {
        return Functions::moyenne_ponderee(
            valeurs: $this->lnc->baies()
                ->map(fn(BaieLnc $baie): float => $baie->data()->ensoleillements->fe())
                ->values(),
    ...
    public function c1(Mois $mois): float
    {
        return Functions::moyenne_ponderee(
            valeurs: $this->lnc->baies()
                ->map(fn(BaieLnc $baie): float => $baie->data()->ensoleillements->c1())
                ->values(),
            coefficients: $this->lnc->baies()
                ->map(fn(BaieLnc $baie): float => $baie->position()->surface)
                ->values(),
    ...
    public function t(Mois $mois): float
    {
        return Functions::moyenne_ponderee(
            valeurs: $this->lnc->baies()
                ->map(fn(BaieLnc $baie): float => $baie->data()->ensoleillements->t($mois))
                ->values(),
            coefficients: $this->lnc->baies()
                ->map(fn(BaieLnc $baie): float => $baie->position()->surface)
                ->values(),
    ...
    public function sst(Mois $mois): float
    {
        return $this->lnc->baies()->reduce(
            fn(float $sst, BaieLnc $baie): float => $sst + $baie->data()->ensoleillements->sst($mois)
        );
    }
